            ...
            <div class="layout-px-spacing">
                ...
                <div class="row layout-top-spacing" id="cancel-row">
                                                    ...
                                                    if ($statement->rowCount() > 0) {
                                                        ...
                                                        while ($user = $statement->fetch()) {
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><?= $user['username'] ?></td>
                                                                <td><?= $user['email'] ?></td>
                                                                <td><a href="" style="background-color:<?= ($user['status']) === "active" ? "green" : "red" ?>;  color:white; padding:10px 15px; height:10px;"><?= $user['status'] ?></a> </td>
                                                                <td><?= $user['registration_time_and_day'] ?></td>
                                                                <td><?= "$" . $user['balance'] ?></td>

                                                                <td>
                                                                    <button class="all_buttons_disable" data-disable="<?= $user['id']  ?>" style="text-decoration:none; border:none;border-radius: 20px; padding: 5px 10px; background-color:red; color: white; cursor: pointer;">Disable</button>
                                                                    <br> <br>
                                                                    <button  data-enable="<?= $user['id'] ?>"   class="all_buttons_enable" style="text-decoration:none;  color: white; border-radius: 20px; padding: 5px 10px; background-color: #3085d6;border:none; cursor: pointer;">Enable</button>
                    ...
                    <script>
                        ...
                        //disable userss
                        document.querySelectorAll('.all_buttons_disable').forEach(button => {
                            button.addEventListener('click', async (e) => {
                                e.preventDefault()
                                try {
                                    const disable_user = e.target.dataset.disable;
                                const response = await fetch("disable.php?disable_user=" + encodeURIComponent(disable_user));
                        ...
                        //enable users
                            document.querySelectorAll('.all_buttons_enable').forEach(button => {
                            button.addEventListener('click', async (e) => {
                                e.preventDefault()
                                try {
                                    const enable_user = e.target.dataset.enable;
                                const response = await fetch("enable_user.php?enable_user=" + encodeURIComponent(enable_user));
